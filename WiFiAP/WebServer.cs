...
namespace WifiAP
{
    public class WebServer
    {
        ...
        private void ProcessRequest(HttpListenerContext context)
        {
            ...
            // Add CORS headers to allow requests from Blazor app
            ...
            switch (request.HttpMethod)
            {
                case "GET":
                    ...
                    else if (url[0] == "/setcolor")
                    {
                        // Handle color setting request
                        string color = null;
                        if (url.Length > 1)
                        {
                            var parameters = ParseParams(url[1]);
                            color = (string)parameters["color"];
                        }

                        if (!string.IsNullOrEmpty(color))
                        {
                            bool success = Program.SetColorByName(color);
                            responseString = success ?
                                $"Color set to {color}" :
                                $"Unknown color: {color}. Available: red, green, blue, yellow, magenta, cyan, white, off";
                        }
                        else
                        {
                            responseString = "Color parameter is required";
                        }
